...
describe('проверяем страницу автора', function() {
    const url = `${baseUrl}${routes.authors}`;
    it(`переход на страницу автора`, function() {
        cy.visit(url); 
        // Находим первую ссылку на странице
        cy.get('[data-cy="author_link_0"]').click();
        // Проверяем, что есть поле ФИО
        cy.get('[data-cy="author_name-label"]').should('be.visible');
        cy.get('[data-cy="author_name-field-readonly"]').should('be.visible');
