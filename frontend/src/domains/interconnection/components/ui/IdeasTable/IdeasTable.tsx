...
import { InterconnectionIdea } from "../../../types/query-types";
...
type IdeasTableProps = {
  sectionClass: string;
  title: string;
  ideas: InterconnectionIdea[];
  isReverse: boolean;
};

export const IdeasTable: FC<IdeasTableProps> = (props: IdeasTableProps) => {
  //return <pre>{JSON.stringify(props.ideas, null, 2)}</pre>
  return (
      ...
      <tbody>
        {props.ideas.map((el) => (
          <tr key={el.interconnection_id}>
            ...
            <td>
              <Link to={genIdeaURL(el.id)} className={styles.idea}>
