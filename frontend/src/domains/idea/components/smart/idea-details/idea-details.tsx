...
import { TopicKeywords } from "../../ui/topic-keywords/topic-keywords";
...
import {
  genHeaderText,
  EditAccessStatus,
  simpleNameFromObject,
  HeaderParType,
  genTabHeaderText,
  isSafeSvg,
  sourceFullNameFromObj,
} from "../../../../../shared/utils/utils";
import styles from "./idea-details.module.scss";
...
export const IdeaDetails: FC<IdeaDetailsProps> = ({
  ...
  const ideaDetailsHook = useIdeaDetails({
    id,
    currentUser,
    findSourceId,
    findKeywordId,
  ...
  const { form, record, status, moderate } = ideaDetailsHook;
  //console.log(' status in IdeaDetails', status);
    ...
    <>
      ...
      >
          ...
          <div className={styles.form_header_inputs}>
            ...
            <InputSelectUI
              ...
              values={record.sources.map((el) => ({
                id: el.id,
                name: el.name + "//" + simpleNameFromObject(el.author),
              }))}
            />
            <DivSpacer />
            <InputEditUI
       ...
       <TopicKeywords
        ...
        <RecordControlBlock
          gotoEntityList={gotoIdeaList}
          gotoEntityEdit={gotoIdea}
          entityDetailsHook={ideaDetailsHook}
        />
