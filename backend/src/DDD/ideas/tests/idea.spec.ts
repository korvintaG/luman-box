...
describe('SourcesService (integration)', () => {
  ...
  it('подготовка данных для тестов - удаление старых данных и добавление нового автора / источника', async () => {
    ...
    await sourceTestHelper.simpleToModerate(tokenUser, sourceID);
    await sourceTestHelper.simpleModerate(tokenAdmin, sourceID, 'approve');
    await keywordTestHelper.simpleToModerate(tokenUser, keywordID);
    await keywordTestHelper.simpleModerate(tokenAdmin, keywordID, 'approve');*/
    entityToAddOKWithSource = { ...entityToAddOK, source: {id: sourceID }, keywords: [{id: keywordID }] };
  });
  ...
  it('проверка невозможности удаления связанных с идеей источника, ключевого слова и автора', async () => {
    ...
    const delRes3=await keywordTestHelper.simpleRemove( tokenSuperAdmin, keywordID, StatusCode.BadRequest);
  });
  ...
  it('удаление идеи, источника, ключевого слова и автора суперадмином', async () => {
      ...
      const delRes3=await keywordTestHelper.simpleRemove( tokenSuperAdmin, keywordID);
  });
