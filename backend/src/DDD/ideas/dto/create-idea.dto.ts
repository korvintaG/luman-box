...
export class CreateIdeaDto {
  ...
  @Length(5, 250, {message: 'Название идеи должно быть от 5-ти до 250 символов длиной'})
  name: string;

  @IsNotEmpty({ message: 'Поле [original_text] не может быть пустым' })
  @IsString({message: 'Поле [original_text] должно быть строкой'})
  @Length(10, 8000, {message: 'Исходная цитата должна быть от 10-ти до 8000 символов длиной'})
  original_text: string;

  @IsNotEmpty({ message: 'Поле $property не может быть пустым' })
  @IsString({message: 'Поле $property должно быть строкой'})
  @Length(30, 4000, {message: 'Идея должна быть от 30-ти до 4000 символов длиной'})
  content: string;
  ...
  @ValidateNested({ each: true })
  @ArrayMinSize(1,{message: 'Для идеи должно быть минимум одно ключевое слово'})
