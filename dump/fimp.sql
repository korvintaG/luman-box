...
Вопрос - а почему бы при жизни не научиться воспринимать в широком диапазоне?', (select id from users where id_out='korvintag_6'), timestamp '11.08.2022 17:24:24' where not exists (select * from sources where id_out='korvintag_260');
...
Т.е. нужна анализировать регулярно моменты творения нашим сознанием нашей реальности. К сожалению, это происходит бесконтрольно для нашего рационального сознаний.', (select id from users where id_out='korvintag_6'), timestamp '11.08.2022 18:31:06' where not exists (select * from sources where id_out='korvintag_261');
...
Кроме того, проблема человека в противоречии между его телом и рассудком. Наверное целостность - это и есть состояние преодоления этого противоречия.', (select id from users where id_out='korvintag_6'), timestamp '11.08.2022 20:46:57' where not exists (select * from sources where id_out='korvintag_264');
...
Тогда может открыться неизведанное.', (select id from users where id_out='korvintag_6'), timestamp '11.08.2022 20:53:54' where not exists (select * from sources where id_out='korvintag_265');
...
Но тональю не исчерпывается человек. В нем есть и другая сущность — нагваль. Так Кастанеда называет часть нас самих, с которой мы вообще не вступаем в контакт. Это нечто, не имеющее определения (ни слов, ни имен, ни ощущений, ни значения), но тем не менее неотторжимый компонент нас самих. Можно оказаться свидетелем нагваля, но определить ее невозможно. Ее способно наблюдать только тело, отнюдь не разум. Нагваль обнаруживается в непостижимых проявлениях.' , 'Проявления Нагваля можно заметить только через тело. Через разум, тональ, нагваля не видно. Его как бы нет. Потому для доступа к Нагвалю нужно лучше изучить свое тело, и обращать внимание на его ощущения.', (select id from users where id_out='korvintag_6'), timestamp '12.08.2022 10:35:11' where not exists (select * from sources where id_out='korvintag_270');
...
- во враждебном пятне тело особым образом реагирует
  ...
  - волосы дыбом на шее 
  - ноги подгинаются
  - пальцы сгинаются', (select id from users where id_out='korvintag_6'), timestamp '12.08.2022 16:55:07' where not exists (select * from sources where id_out='korvintag_274');
...
— Ты попал в точку, дон Хуан. Но как мне измениться?' , 'Серьезность не должна быть привязана только к тому, что мы делаем. Серьезно нужно относиться и к тому, что происходит вне нас. Односторонняя серьезность порождает сильную усталость. Именно усталость не дает нам замечать чудеса.', (select id from users where id_out='korvintag_6'), timestamp '11.08.2023 11:14:21' where not exists (select * from sources where id_out='korvintag_2252');
...
— Это произойдет мало-помалу, но страх исчезнет внезапно и быстро.' , 'Страх - первый враг на пути к знанию. При столкновении с ним нужно идти дальше, невзирая на страх. И он уйдет. И появится уверенность.', (select id from users where id_out='korvintag_6'), timestamp '11.08.2023 16:32:42' where not exists (select * from sources where id_out='korvintag_2259');
...
Эту ошибку нужно преодолеть. Ни в коей мере не следует ей сдаваться.', (select id from users where id_out='korvintag_6'), timestamp '11.08.2023 16:38:10' where not exists (select * from sources where id_out='korvintag_2260');
...
- отказывается от самого себя', (select id from users where id_out='korvintag_6'), timestamp '11.08.2023 19:17:01' where not exists (select * from sources where id_out='korvintag_2262');
...
Попасть в это состояние нам мешает разум, поскольку он очень громкий.', (select id from users where id_out='korvintag_109'), timestamp '27.12.2023 18:38:34' where not exists (select * from sources where id_out='korvintag_3852');
